...
void CanopyEngine::RenderFrame(unsigned long width, unsigned long height,
  ...
  DrawRectWithBounds(
    kBorderSize.width,
    kBorderSize.height,
    frame_width - kBorderSize.width,
    frame_height - kBorderSize.height,
    kArenaColor);
  DrawRectAbsolute(game_state.GetBallPosition(), kBallSize, kBallColor);
  DrawRectAbsolute(
    game_state.GetPlayerPosition(0),
    kPlayerSize,
    kPlayerColor);
  DrawRectAbsolute(
    game_state.GetPlayerPosition(1),
    kPlayerSize,
    kPlayerColor);
}
...
void CanopyEngine::DrawRectWithBounds(int left, int top, int right, int bottom,
  ...
  for (int y = top; y < bottom; y++) {
    unsigned int *pixel =
      static_cast<unsigned int *>(this->pixel_buffer) + left + y * frame_width;
